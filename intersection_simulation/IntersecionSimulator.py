...
# 0~ 23 시 시간단위로 차의 비율.
# : 하루 다니는 차의 수 * 시간 으로 해당 시간에 움직이는 차의 갯수 가져옴 .
hourlyRates = [0.0081, 0.0052, 0.0047, 0.0057, 0.0099, 0.0230, 0.0489, 0.0679, 0.0629, 0.0531, 0.0509, 0.0538, 0.0560,
               0.0574, 0.0635, 0.0733, 0.0804, 0.0775, 0.0579, 0.0437, 0.0338, 0.0280, 0.0205, 0.0138]
...
controlDevice = "Light"
departRate = 2
# how quickly can they go once it is their turn in seconds.
# 초단위로 봤을 때, 한번에 걔네들이 얼마나 빨리 갈 수 있는가 ?
    ...
    while True:
        ...
        if len(signOrder) < 4:
            ...
            try:
                ...
                ## 네방향을 sign order 에 업데이트 하는 걸, 굳이 이렇게 해야됨? 그냥 처음 부터 NESW 집어 넣으면 안되나?
        if dir == "N":
...
def main(outFile):
    ...
    for e in eventLog:
        ...
        file.write(
            str(carNum) + "," + str(arrival) + "," + str(departure) + "," + str(waitTime) + "," + direction + "\n")
